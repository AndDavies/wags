...
// Vercel: Explicitly set runtime to Node.js (Serverless)
...
export async function POST(request: NextRequest) {
  console.log('[ChatGenerateItinerary API] Received POST request'); // Log specific endpoint
  let tripData: TripData;

  try {
    // --- Step 1: Parse and Validate Input --- 
    console.log('[ChatGenerateItinerary API] Parsing request body...');
    try {
      tripData = await request.json();
      console.log('[ChatGenerateItinerary API] Request body parsed:', tripData);
    } catch (parseError) {
      console.error('[ChatGenerateItinerary API] Failed to parse request JSON:', parseError);
      return NextResponse.json({ error: 'Invalid request body: Malformed JSON.' }, { status: 400 });
    }

    console.log('[ChatGenerateItinerary API] Validating essential fields...');
    // *** MODIFIED: Validate only essential fields for chat generation ***
    const essentialFields: (keyof TripData)[] = ['destination', 'destinationCountry', 'startDate', 'endDate']; 
    ...
    if (missingEssentialFields.length > 0) {
      console.error('[ChatGenerateItinerary API] Validation failed - Missing essential fields:', missingEssentialFields);
      return NextResponse.json(
        { error: `Missing essential fields required for generation: ${missingEssentialFields.join(', ')}` },
        { status: 400 }
      );
    }
    console.log('[ChatGenerateItinerary API] Essential fields validated.');

    // --- Step 1b: Apply Smart Defaults for Missing Optional Fields ---
    console.log('[ChatGenerateItinerary API] Applying smart defaults...');
    // Basic user info
    tripData.origin = tripData.origin || "User's Location";
    tripData.originCountry = tripData.originCountry || "User's Country";
    tripData.adults = typeof tripData.adults === 'number' ? tripData.adults : 1; // Default to 1 adult
    tripData.children = typeof tripData.children === 'number' ? tripData.children : 0; // Default to 0 children

    // Pet info
    tripData.pets = typeof tripData.pets === 'number' ? tripData.pets : 1; // Default to 1 pet if undefined
    // If pets > 0 but details are missing or empty, add a default pet
    if (tripData.pets > 0 && (!tripData.petDetails || tripData.petDetails.length === 0)) {
      tripData.petDetails = [{ type: 'Dog', size: 'Medium' }];
      console.log('[ChatGenerateItinerary API] Defaulted petDetails:', tripData.petDetails);
    } else if (tripData.pets === 0) {
        tripData.petDetails = []; // Ensure petDetails is empty if pets is 0
    }

    // Trip preferences
    tripData.budget = tripData.budget || 'Moderate';
    tripData.accommodation = tripData.accommodation || 'Hotel';
    tripData.interests = (tripData.interests && tripData.interests.length > 0) 
        ? tripData.interests 
        : ['Outdoor Adventures', 'Sightseeing', 'Food Tours']; // Default interests

    // Other optional fields
    tripData.additionalCities = tripData.additionalCities || [];
    tripData.additionalInfo = tripData.additionalInfo || '';
    
    console.log('[ChatGenerateItinerary API] TripData after defaults applied:', tripData);
    
    // Calculate trip duration (Moved after defaults to ensure dates are present)
   // console.log('[EnhancedItinerary API] Calculating trip duration...');
    ...
    const getUniqueActivity = (pool: Activity[]): Activity | undefined => {
        while (pool.length > 0) {
            ...
            // Use place_id for uniqueness check if available, fallback to location
            const uniqueKey = activity?.place_id || activity?.location;
            if (activity && uniqueKey && !usedActivityPlaceIds.has(uniqueKey)) {
                usedActivityPlaceIds.add(uniqueKey); // Add the valid key
                return activity;
            }
        }
        return undefined;
    ...
    const getUniqueRestaurant = (pool: Activity[]): Activity | undefined => {
         while (pool.length > 0) {
            const restaurant = pool.shift();
             // Use place_id for uniqueness check
            const uniqueKey = restaurant?.place_id || restaurant?.location;
            if (restaurant && uniqueKey && !usedActivityPlaceIds.has(uniqueKey)) {
                usedActivityPlaceIds.add(uniqueKey); // Add the valid key
                return restaurant;
            }
        }
    ...
    if (tripData.pets > 0) {
        // Use the fully typed tripData here, providing fallback just in case
        preDeparturePreparation.push({
            name: "Veterinarian Visit & Paperwork",
            description: "Schedule a vet appointment 1-2 weeks before departure... [Review Best Practices](/blog/best-practices)",
            petFriendly: true,
            location: tripData.origin || 'User\'s Location', // Fallback added
        ...
        preDeparturePreparation.push({
            ...
            description: "Confirm pet booking with your airline... [Check Airline Policies](/blog/airline-pet-policies)",
            petFriendly: true,
            location: tripData.origin || 'User\'s Location', // Fallback added
            coordinates: { lat: 0, lng: 0 },
            cost: "Varies",
    ...
    const defaultCoords = destinationAirportCoords || { lat: 0, lng: 0 };

    // Use the fully typed tripData here, providing fallback
    day1Activities.push({
      name: "Final Travel Prep",
      description: `Check flight status. Ensure all pet documents... Confirm airline's specific pet check-in procedure.`,
      petFriendly: true,
      location: tripData.origin || 'User\'s Location', // Fallback added
      coordinates: { lat: 0, lng: 0 },
      startTime: "08:00",
    ...
    day1Activities.push({
      ...
      description: `Flight from ${tripData.origin || 'Unknown Origin'}. Proceed through...`, // Fallback added
      petFriendly: true,
      location: `${tripData.destination} Airport Area`,
    ...
    if (!day1LunchActivity) {
        ...
        const lunchOption = nearbyLunchOptions.find(p => p.place_id && !usedActivityPlaceIds.has(p.place_id));
        if (lunchOption && lunchOption.place_id) { // Check place_id exists before using
            day1LunchActivity = placeToActivity(lunchOption, 'meal');
            usedActivityPlaceIds.add(lunchOption.place_id); // Add the guaranteed place_id
        } else {
    ...
    // Ensure activity exists before pushing
    if (day1LunchActivity) {
    ...
    if (!day1AfternoonActivity || !['park', 'hiking'].some(type => day1AfternoonActivity!.description.toLowerCase().includes(type))) {
        ...
        const parkOption = nearbyParkOptions.find(p => p.place_id && !usedActivityPlaceIds.has(p.place_id));
        if (parkOption && parkOption.place_id) { // Check place_id exists before using
            day1AfternoonActivity = placeToActivity(parkOption, 'activity');
            usedActivityPlaceIds.add(parkOption.place_id); // Add the guaranteed place_id
        } else {
    ...
    if (!day1DinnerActivity) {
        ...
        const dinnerOption = nearbyDinnerOptions.find(p => p.place_id && !usedActivityPlaceIds.has(p.place_id));
        if (dinnerOption && dinnerOption.place_id) { // Check place_id exists before using
            day1DinnerActivity = placeToActivity(dinnerOption, 'meal');
            usedActivityPlaceIds.add(dinnerOption.place_id); // Add the guaranteed place_id
        } else {
    ...
    itinerary.days.push({
      ...
      travel: `Travel day from ${tripData.origin || 'User\'s Location'} to ${tripData.destination}.`, // Fallback refined
